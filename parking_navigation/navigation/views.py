        ...
        case 'POST':
            ...
            if extend_minutes:
                if user_profile.exit_time:
                    current_exit = datetime.combine(timezone.now().date(), user_profile.exit_time)
                    updated_exit = current_exit + timedelta(minutes=int(extend_minutes))
                    user_profile.exit_time = updated_exit.time()
                    user_profile.save()
...
def parking_status(request):
    ...
    if user_profile.arrival_time and user_profile.exit_time:
        ...
        if 0 < remaining_seconds <= 600:  # Less than or equal to 10 mins
            alert_needed = True
